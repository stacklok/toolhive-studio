...
4. **load_skill({ name })** — Resolves the on-disk install of an installed skill and reads \`SKILL.md\`. Returns \`{ name, version, scope, projectRoot?, client, dir, body, files, cached }\`. \`scope\` is \`"user"\` (\`~/.<client>/skills/<name>/\`) or \`"project"\` (\`<projectRoot>/.<client>/skills/<name>/\`); \`projectRoot\` is present only when \`scope === "project"\`. \`body\` is the raw SKILL.md, \`files\` is a recursive listing of bundled resources (paths relative to \`dir\`), \`client\` is which AI client's install dir we read from. When a skill is installed in multiple places, resolution prefers the user home, then falls back to each project in backend order. Call exactly ONCE per skill before any \`read_skill_file\` / \`list_skill_tree\`.

...
interface FormRunFromRegistryProps<T extends RegistryRemoteServerMetadata> {
  server: T | null
  isOpen: boolean
  closeDialog: () => void
}
...
export function DialogFormRemoteRegistryMcp<
  T extends RegistryRemoteServerMetadata,
>({ server, isOpen, closeDialog }: FormRunFromRegistryProps<T>) {
  ...
  const registryData = convertCreateRequestToFormData(
    server,
  ...
  const form = useForm<FormSchemaRemoteMcp>({
    ...
    mode: 'onChange',
    ...(registryData ? { values: registryData } : {}),
  })

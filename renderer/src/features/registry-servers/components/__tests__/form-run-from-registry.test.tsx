...
describe('FormRunFromRegistry', () => {
  ...
  describe('Storage volumes field', () => {
    it('renders storage volumes field with add button', async () => {
      const server = { ...REGISTRY_SERVER }
      server.env_vars = ENV_VARS_OPTIONAL
      ...
      expect(screen.getByText('Storage volumes')).toBeInTheDocument()
      expect(screen.getByText('Add storage volume')).toBeInTheDocument()
    })

    it('allows adding a storage volume with local and container paths', async () => {
      const server = { ...REGISTRY_SERVER }
      server.env_vars = ENV_VARS_OPTIONAL
    ...
    it('allows adding multiple storage volumes', async () => {
      const server = { ...REGISTRY_SERVER }
      server.env_vars = ENV_VARS_OPTIONAL
      ...
      await waitFor(() => {
        ...
        expect(screen.getByLabelText('Container path 2')).toBeInTheDocument()
      })
    ...
    it('allows removing storage volumes', async () => {
      const server = { ...REGISTRY_SERVER }
      server.env_vars = ENV_VARS_OPTIONAL
      ...
      const removeButton = screen.getByRole('button', {
        name: /remove storage volume/i,
      })
      await userEvent.click(removeButton)

      await waitFor(() => {
        expect(screen.queryByLabelText('Local path 1')).not.toBeInTheDocument()
        expect(
          screen.queryByLabelText('Container path 1')
        ).not.toBeInTheDocument()
      })
    })

    it('allows typing in storage volume fields', async () => {
      const server = { ...REGISTRY_SERVER }
      server.env_vars = ENV_VARS_OPTIONAL
      ...
      expect(localPathInput).toHaveValue('/home/user/data')
      expect(containerPathInput).toHaveValue('/app/data')
    })

    it('submits form with storage volumes data', async () => {
    ...
    it('submits form with multiple storage volumes', async () => {
      const mockInstallServerMutation = vi.fn()
      mockUseRunFromRegistry.mockReturnValue({
        installServerMutation: mockInstallServerMutation,
        checkServerStatus: vi.fn(),
    ...
    it('focuses input when adding new storage volume', async () => {
      const server = { ...REGISTRY_SERVER }
      server.env_vars = ENV_VARS_OPTIONAL
      ...
      const localPathInput = screen.getByLabelText('Local path 1')
      expect(localPathInput).toHaveFocus()
    })

    it('maintains focus after removing storage volume', async () => {
      const server = { ...REGISTRY_SERVER }
      server.env_vars = ENV_VARS_OPTIONAL
      ...
      // Focus on second storage volume
      const secondLocalPathInput = screen.getByLabelText(
        'Local path 2'
      ) as HTMLInputElement
      secondLocalPathInput.focus()

      // Remove second storage volume
      const removeButtons = screen.getAllByRole('button', {
        name: /remove storage volume/i,
      })
      await userEvent.click(removeButtons[1]) // Remove the second one

      await waitFor(() => {
        expect(screen.queryByLabelText('Local path 2')).not.toBeInTheDocument()
      })

      // Check that focus is maintained on the add button

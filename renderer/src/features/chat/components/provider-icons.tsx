...
function getProviderIdFromModel(model: string): string | null {
  ...
  // Note: Ollama and LM Studio models are user-defined and can have any names,
  // so we can't reliably detect them from model name alone.
  // The providerId is now stored in message metadata for accurate provider icons.
  return null
}

// Get provider icon by model and optional providerId from metadata (without tooltip)
export function getProviderIconByModel(
  ...
  // Otherwise, try to infer from model name (for backwards compatibility)
  const inferredProviderId = getProviderIdFromModel(model)
  if (!inferredProviderId) return null
  return getProviderIconWithTooltip(inferredProviderId) || null
}

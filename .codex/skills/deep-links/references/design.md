...
_Part of the [deep-links skill](../SKILL.md). Contains design rationale, finalized decisions, and the minimal proof of concept scope. **Some sections describe planned future behaviour not yet implemented** — see the base skill for what is currently in the codebase._
...
- toolhive-gui://v1/open-registry _Open registry page_
- toolhive-gui://v1/run-local-server?serverName=fetch _Run the fetch server locally from the registry_
